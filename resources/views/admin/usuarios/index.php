...
$militarLabels = ['sim' => 'Militar', 'nao' => 'Nao militar'];
...
$filterLabels = ['q' => 'Busca', 'perfil' => 'Perfil', 'status' => 'Status', 'militar' => 'Vinculo'];
...
<section class="records-page users-page">
    ...
    <section class="records-summary-grid users-summary-grid" aria-label="Resumo dos usuarios">
        ...
        <article class="records-summary-card">
            ...
            <small><?= $ultimoAcesso !== null ? 'Ultimo acesso em ' . h(date('d/m/Y H:i', $ultimoAcesso)) : 'Sem acesso registrado' ?></small>
        </article>
    </section>
    ...
    <div class="records-list-toolbar">
        <span><?= h($totalUsuarios) ?> usuario(s) encontrado(s)</span>
        <strong><?= h($firstRecord) ?>-<?= h($lastRecord) ?> de <?= h($totalUsuarios) ?></strong>
    </div>

    <?php if ($usuarios === []): ?>
        <section class="action-empty-panel records-empty-panel">
            <h2><?= $activeFilters === [] ? 'Nenhum usuario cadastrado' : 'Nenhum usuario encontrado' ?></h2>
            ...
            <?php else: ?>
                <a class="primary-link-button" href="<?= h(url('/admin/usuarios/novo')) ?>">Criar primeiro usuario</a>
            <?php endif; ?>
        </section>
    <?php else: ?>
        <section class="user-card-list" aria-label="Lista de usuarios">
                ...
                <article class="user-card">
                    <div class="user-card-main">
                        ...
                        <dl class="user-card-meta">
                            ...
                            <div><dt>Orgao</dt><dd><?= h($usuario['orgao'] ?: '-') ?></dd></div>
                            <div><dt>Unidade</dt><dd><?= h($usuario['unidade_setor'] ?: '-') ?></dd></div>
                            <div><dt>Ultimo acesso</dt><dd><?= $ultimo !== false ? h(date('d/m/Y H:i', $ultimo)) : '-' ?></dd></div>
                    ...
                    <aside class="user-card-actions" aria-label="Acoes do usuario">
                        ...
                        <?php if ($ativo): ?>
                            ...
                            <?php else: ?>
                                <form method="post" action="<?= h(url('/admin/usuarios/' . $usuario['id'] . '/inativar')) ?>" class="inline-form js-prevent-double-submit" data-confirm="Inativar este usuario? Ele nao conseguira acessar o sistema.">
                                    <?= csrf_field() ?>
    ...
    <?php if ($totalPages > 1): ?>
        <nav class="records-pagination" aria-label="Paginacao dos usuarios">
