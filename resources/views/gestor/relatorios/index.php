...
$documentLabels = [
    ...
    'responsavel_documento' => 'Doc. responsável',
...
<section class="records-page report-page">
    <header class="dashboard-header records-header report-hero no-print">
        <div>
            ...
            <p>Leitura consolidada dos cadastros, famílias, entregas, documentos, assinaturas e elegibilidade do Programa Recomeçar.</p>
        </div>
    ...
    <section class="records-filter-panel report-filter-panel report-filter-modern-panel no-print" aria-label="Filtros de relatórios">
        <div class="table-heading">
            <div>
                <h2>Filtros inteligentes</h2>
                <span>Combine ação, texto, status, tipo de ajuda, período e situação cadastral para montar uma visão operacional precisa.</span>
            </div>
        </div>
        ...
        <form method="get" action="<?= h(url('/gestor/relatorios')) ?>" class="report-filter-form report-filter-modern-form">
            <label class="field styled-field report-filter-field report-filter-field-wide">
                <span>Busca</span>
                <input type="search" name="q" value="<?= h($filters['q'] ?? '') ?>" maxlength="180" placeholder="Protocolo, família, CPF, telefone, ação ou endereço">
            </label>

            <label class="field styled-field smart-search-field report-filter-field report-filter-field-wide">
                <span>Ação emergencial</span>
                <input type="search" name="acao_busca" value="<?= h(($filters['acao_busca'] ?? '') !== '' ? $filters['acao_busca'] : $selectedActionLabel) ?>" list="relatorio-acoes-list" placeholder="Digite município, localidade, evento, status ou ação #ID" data-smart-search data-smart-target="relatorio_acao_id" autocomplete="off">
                ...
                <datalist id="relatorio-acoes-list">
                    <?php foreach ($acoes as $acao): ?>
                        ...
                        <option value="<?= h('Ação #' . $acao['id'] . ' - ' . $acaoLabel) ?>" data-id="<?= h($acao['id']) ?>"></option>
                    <?php endforeach; ?>
                </datalist>
            </label>
    ...
    <section class="report-document">
        <div class="print-heading">
            ...
            <p>Período: <?= h($periodo) ?> | Gerado em <?= h($generatedAt->format('d/m/Y H:i')) ?></p>
        </div>

        <section class="report-kpi-grid" aria-label="Indicadores do relatório">
            ...
            <article class="report-kpi-card">
                ...
                <small><?= h($formatPercent($cadastrosConcluidos, $familias)) ?> com cadastro concluído</small>
            </article>
            <article class="report-kpi-card">
                <span>Pessoas</span>
            ...
            <article class="report-kpi-card">
                ...
                <small>família(s) apta(s) ao programa no recorte</small>
            </article>
        </section>

        <section class="report-insight-grid no-print" aria-label="Leituras rápidas">
            <article class="report-insight-card">
                <span>Prioridade de campo</span>
                <strong><?= h($familiasPendentes > 0 ? $formatInt($familiasPendentes) . ' família(s) sem entrega' : 'Sem pendências de entrega') ?></strong>
                <small>Use a lista de pendências no fim do relatório para acionar equipes de distribuição.</small>
            </article>
            <article class="report-insight-card">
                ...
                <small>Cadastros completos reduzem retrabalho em prestação de contas, DTI e Recomeçar.</small>
            </article>
            <article class="report-insight-card">
        ...
        <section class="table-panel report-section">
            <div class="table-heading">
                <h2>Resumo por ação emergencial</h2>
                <span><?= h(count($byAction)) ?> ação(ões)</span>
            </div>
            <table class="data-table report-data-table">
                ...
                <tbody>
                    <?php foreach ($byAction as $item): ?>
                        <tr>
                            <td data-label="Município"><?= h($item['municipio_nome']) ?>/<?= h($item['uf']) ?></td>
                            <td data-label="Ação"><?= h($item['localidade']) ?> - <?= h($item['tipo_evento']) ?></td>
                            ...
                            <td data-label="Residências"><?= h($formatInt($item['residencias'])) ?></td>
                            <td data-label="Famílias"><?= h($formatInt($item['familias'])) ?></td>
        ...
        <section class="report-panel-grid">
            <article class="table-panel report-section">
                ...
                <table class="data-table compact-table report-data-table">
                    <thead><tr><th>Bairro/comunidade</th><th>Famílias</th><th>Pessoas</th></tr></thead>
                    <tbody>
            ...
            <article class="table-panel report-section">
                <div class="table-heading">
                    <h2>Imóvel e dano</h2>
                    <span>Classificação do cadastro</span>
                </div>
                <div class="report-mini-tables">
                    <table class="data-table compact-table report-data-table">
                        <thead><tr><th>Imóvel</th><th>Residências</th><th>Famílias</th></tr></thead>
                        <tbody>
                            <?php foreach ($byHousingType as $item): ?>
                                <tr>
                                    <td data-label="Imóvel"><?= h(residencia_imovel_label($item['imovel'] ?? null)) ?></td>
                                    <td data-label="Residências"><?= h($formatInt($item['residencias'])) ?></td>
                                    <td data-label="Famílias"><?= h($formatInt($item['familias'])) ?></td>
                                </tr>
                            <?php endforeach; ?>
                    ...
                    <table class="data-table compact-table report-data-table">
                        <thead><tr><th>Condição</th><th>Residências</th><th>Famílias</th></tr></thead>
                        <tbody>
                            <?php foreach ($byResidenceCondition as $item): ?>
                                <tr>
                                    <td data-label="Condição"><?= h(residencia_condicao_label($item['condicao_residencia'] ?? null)) ?></td>
                                    <td data-label="Residências"><?= h($formatInt($item['residencias'])) ?></td>
                                    <td data-label="Famílias"><?= h($formatInt($item['familias'])) ?></td>
                                </tr>
                            <?php endforeach; ?>
        ...
        <section class="report-panel-grid">
            <article class="table-panel report-section">
                ...
                <table class="data-table compact-table report-data-table">
                    <thead><tr><th>Ajuda</th><th>Famílias</th><th>Total</th></tr></thead>
                    <tbody>
                        <?php foreach ($deliveriesByType as $item): ?>
                            <tr>
                                <td data-label="Ajuda"><?= h($item['nome']) ?></td>
                                <td data-label="Famílias"><?= h($formatInt($item['familias_atendidas'])) ?></td>
            ...
            <article class="table-panel report-section">
                <div class="table-heading">
                    <h2>Últimos dias de entrega</h2>
                    <span><?= h(count($deliveryTimeline)) ?> dia(s)</span>
                </div>
                <table class="data-table compact-table report-data-table">
                    <thead><tr><th>Data</th><th>Famílias</th><th>Entregas</th><th>Qtd.</th></tr></thead>
                    <tbody>
        ...
        <section class="report-card-grid">
            ...
            <article class="report-metric-panel">
                ...
                <div class="report-meter-row"><span>Famílias concluídas</span><strong><?= h($formatPercent($registrationQuality['familias_concluidas'] ?? 0, $registrationQuality['familias'] ?? 0)) ?></strong></div>
                ...
                <div class="report-meter-row"><span>Com e-mail</span><strong><?= h($formatPercent($registrationQuality['familias_com_email'] ?? 0, $registrationQuality['familias'] ?? 0)) ?></strong></div>
                <div class="report-meter-row"><span>Residências georreferenciadas</span><strong><?= h($formatPercent($registrationQuality['com_geolocalizacao'] ?? 0, $registrationQuality['residencias'] ?? 0)) ?></strong></div>
            </article>
            ...
            <article class="report-metric-panel">
                ...
                <?php endforeach; ?>
                <?php if ($signatureStats === []): ?><p class="empty-state">Sem assinaturas no período.</p><?php endif; ?>
            </article>
        </section>
            ...
            <table class="data-table report-data-table">
                ...
                <tbody>
                    ...
                    <?php if ($pendingFamilies === []): ?>
                        <tr><td colspan="5" class="empty-state">Nenhuma pendência encontrada.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </section>
